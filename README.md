...
3. Add the 8 relationships using the bridge tables specified in the Ontology section above
...
- New columns in `PhoneticContext`: `phrase_position`, `information_status` (new vs given information)
...
When two or more adjacent segments trigger flapping in rapid succession (e.g. "get rid of it"), the agent may collapse them into a single flap event rather than tracing each independently. In "get rid of it", the /t/ in "get" and the /d/ in "rid" should each produce separate flaps — [ɡɛɾ ɹɪɾ əv ɪt] — but the agent merged them into a single [ɡɛɾ̬əv], effectively dropping "rid" from the analysis. Corrected by adding explicit multi-flap chain examples to `AnnotatedSegment` and `Transformation`.
...
When asked to analyze Australian English ("How are you going?"), the agent attempted a response instead of acknowledging that AuE is outside its current scope. The `SpeechVariety` table includes AuE and CanE as placeholders but neither has associated rules in `PronunciationRule` or `RuleVariety`. The updated agent instructions now explicitly block analysis of these varieties and instruct the agent to acknowledge the gap rather than generating plausible-sounding but unverified output. The schema is designed for extensibility — adding a new variety requires only new rows in `SpeechVariety`, `RuleVariety`, and `PronunciationRule`, with no schema changes.
...
Links are constructed dynamically by the agent for any input sentence — not limited to the 10 example sentences in the corpus. The `AudioReference` table stores curated links for the seed corpus; dynamic links extend coverage to every sentence the agent analyzes.
...
- **Australian and Canadian English** rules — `SpeechVariety` and `RuleVariety` are designed for extensibility; adding new varieties requires only new rows, not schema changes
